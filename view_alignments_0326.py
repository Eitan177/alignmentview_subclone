...
def view_alignment(aln, useconsensus,fr=1,fontsize="9pt", plot_width=750,see_seq=False):
    ...
    elif len(set(alnscores)) > 11:
        subclonecolors=Viridis[11]+Cividis[len(set(alnscores))-11]
    else:
        subclonecolors=("green","purple")[0:len(set(alnscores))]

    colors=get_colors_consensus(seqs,consensus_sequence)
    
    gg=np.array(colors).reshape((-1, len(seqs[0])))
    locofinsertion=[int(jj.split(',')[0].replace('[',''))  for jj in forfileinsert[np.where(forfileinsert != '[]')]]
    ...
    gg[np.where(forfileinsert != '[]'),locofinsertion]='black'
    ...
    st.download_button(label='proper order Download',data=convert_df(pd.DataFrame({'sequence': seqs,'sequenceNotformatted': forfileseqs,'numberobserved':counts,'inserts':forfileinsert.tolist()})),file_name='unique_data_'+str(fr)+'.csv',mime='text/csv') 
